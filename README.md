...
**1 枚の画像 / 動画から、リグ付き 3D キャラクターとモーション FBX / BVH を作る** ための Web UI アプリケーション。ComfyUI を介さずに FastAPI + Three.js で動くスタンドアロンサーバーとして動作します。

- 画像タブ: 画像 → SAM3 人物マスク → SAM 3D Body ポーズ推定 → 任意体型 3D 素体レンダ → リグ付き FBX / 1 フレーム BVH 書き出し
- 動画タブ: 動画 → 各フレームのポーズ推定 → モーション付き FBX / 全フレーム BVH 書き出し (Blender subprocess 経由)
...
1 枚の画像から、任意の体型の 3D 素体にポーズを当てはめてレンダリング・FBX / BVH 書き出しするタブ。

- 画像アップロード → SAM3 で人物領域をマスキング (`text_prompt = "person"` がデフォルト)。透過 PNG (RGBA / LA / 透過付きパレット) は推定前に白背景へ合成してから処理。
- SAM 3D Body で全身ポーズを推定
- **前かがみ補正** スライダー (0〜1、既定 0) で上半身が前のめりに予測された場合に spine→neck チェーンを後ろに反らす
- 下段の **Character パネル** で preset を選択するか、既存のキャラクター JSON をドロップして体型を指定
- Three.js でリアルタイムにレンダリングプレビュー
- **ポーズ調整 (回転・移動)** ボタンで詳細ボーン編集モードに入れる:
  - 3D ビュー上のボーン (青いスフィア) をクリックして選択 → 出てくる移動ギズモ (X/Y/Z 矢印) をドラッグすると **CCD IK** で親ボーンが追従
  - 主要 22 ボーン + 両手指 (5 指 × 4 関節 = 40) + 両つま先 = 計 64 ボーン
  - サイドパネルの **X / Y / Z スライダー** で選択中ボーンの軸別細かい回転
  - **Hips を選択してドラッグ** すると全身を平行移動 (CCD ではなく translate)
  - 膝・肘は過伸展を自動で防止 (kneecap が後ろを向く方向に折れない)
  - **Undo / Redo** (Ctrl+Z / Ctrl+Y、または専用ボタン)。1 D&D = 1 ステップ
  - `このボーンをリセット` / `全ボーンをリセット` は **そのセッション開始時の状態** に戻す (前セッションの調整は保持)
  - 設定した回転は FBX / BVH に焼き込まれる。再ポーズ推定時 (`/api/process`) は IK 履歴ごとリセット
- `FBX をダウンロード` ボタンで Blender subprocess (`tools/build_rigged_fbx.py`) が起動して rigged FBX を生成
- `BVH をダウンロード` で同じ rigged FBX を作ってから `tools/fbx2bvh_simple.py` で **1 フレーム BVH** に変換して保存
- Blender は FBX / BVH どちらの書き出しにも必須
- 体型 (`bone_lengths`)・ポーズ調整 (`rotation_overrides`)・前かがみ補正は **すべて FBX / BVH の出力に反映** される

### 🎬 2. 動画タブ

動画から連続モーションを抽出して、アニメーション FBX / BVH として書き出すタブ。

- 動画アップロード → 各フレームを SAM 3D Body に通してジョイント回転を推定
- 下段の **Character パネル** で preset を選択すると、その体型でリグ付けされたキャラクターに全フレームがキーフレームとしてベイクされる
- **前かがみ補正** スライダー (0〜1、既定 0) を全フレームの rig に適用 (スライダーを離したタイミングで再ビルド)
- `root_motion_mode` で地面ロック補正を選択 (`auto_ground_lock` / `free` / `xz_only`)
- `bbox thr` / `fps` / `stride` / `max frames` などの推定パラメータを詳細調整
- Blender subprocess (`tools/build_animated_fbx.py`) でアニメーション FBX を書き出し
- `BVH をダウンロード` で現在の animated FBX を `tools/fbx2bvh_simple.py` に通して **全フレーム BVH** に変換
