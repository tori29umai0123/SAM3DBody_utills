...
| Blender | 4.1.1 (`setup.sh` / `setup.cmd` がポータブル版を自動 DL するのでインストール不要) |
...
`setup.cmd` は以下を自動で行います:
- [uv](https://docs.astral.sh/uv/) による `.venv` (Python 3.11) 作成
- `pyproject.toml` / `uv.lock` に基づいた依存のインストール (torch/torchvision/torchaudio は cu128 ホイール)
- 特殊 CUDA wheel は `wheels/` 配下からインストール
- 公式ポータブル **Blender 4.1.1** (`blender-4.1.1-windows-x64.zip`, 約 400 MB) を `blender41-portable/` に自動 DL・展開 (既に配置済みならスキップ)
...
`setup.sh` はプラットフォーム別に以下を自動で行います:

| アーキ | torch/triton | ポータブル Blender |
|---|---|---|
| x86_64 | cu128 stable (pypi 経由) | 公式 `blender-4.1.1-linux-x64.tar.xz` を `download.blender.org` から自動 DL → `blender41-portable/` |
| aarch64 (ARM64) | nightly cu130 + 専用 triton wheel (uv sources で platform 条件分岐) | 自前ビルド版を本リポジトリの [GitHub Releases (`blender-arm64-v1.0`)](https://github.com/tori29umai0123/SAM3DBody_utills/releases/tag/blender-arm64-v1.0) から自動 DL (sha256 検証つき) → `ARM_blender41-portable/` |

いずれも Blender 展開後に `site-packages/` に `numpy` を注入します (自前ビルド Blender は numpy を同梱しないため FBX exporter が落ちるのを回避)。
...
`config.ini` では Blender パスを扱いません。`setup.sh` / `setup.cmd` がプロジェクト直下に配置するポータブル版を相対パスで解決します (優先順):

1. 環境変数 `SAM3DBODY_BLENDER_EXE` (エスケープハッチ、明示指定したいとき用)
2. プロジェクト同梱のポータブル Blender
...
├── config.ini
├── pyproject.toml
├── run.cmd / run.sh                ← uvicorn 起動
├── setup.cmd / setup.sh            ← uv + 依存インストール
