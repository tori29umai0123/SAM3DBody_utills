...
- **手画像による上書き入力**:
  - メイン画像入力の下に **Left hand image** / **Right hand image** の D&D 入力欄があり、それぞれの右に「左右反転」ボタンが付属。
  - 反転ボタンは canvas 上で実ピクセルを反転するため、サーバ送信時にもそのまま反映されます。
  - 手画像をセットして「ポーズを推定」を押すと、本体ポーズに加えて hand-only decoder で手のポーズだけを別解析し、`hand_pose_params[:54]` (左) / `[54:]` (右) を上書きします。手首の向きは本体ポーズのまま。
- **指定範囲撮影モード** (旧「PNG 保存」を改名):
  - 3D ビュー左上の **「指定範囲撮影」** ボタンでモードに入ると、ボタンが **「撮影終了」** に変化し、その下に `撮影` / `背景色指定` / `リセット` の 3 ボタンと、赤い半透明の D&D 範囲指定オーバーレイが現れます。
  - モード中はサイドバー / タブナビが pointer-events ロック。これらのボタンと範囲ドラッグのみ操作できます。
  - **撮影**: 範囲が指定されていればその範囲のみ、未指定ならビュー全体を、指定した背景色 (デフォルト白) で PNG 保存します。
  - **背景色指定**: カラーピッカーで撮影 PNG の背景色を変更。
  - **リセット**: 赤枠 (範囲指定) のみクリア。
  - **撮影終了**: モードを抜けると各種ボタンと赤オーバーレイは消え、ボタンラベルが「指定範囲撮影」に戻ります。範囲指定もリセットされます。
